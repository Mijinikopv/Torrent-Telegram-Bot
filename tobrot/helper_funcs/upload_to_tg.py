...
async def upload_to_gdrive(file_upload, message, messa_ge, g_id):
    ...
    del_it = await message.edit_text("Now Upload To ☁️GoogleDrive☁️")
    ...
    if os.path.isfile(file_upload):
        ...
        print(gjay)
        button = []
        button.append([pyrogram.InlineKeyboardButton(text="☁️ Google Drive ☁️", url=f"{gau_link}")])
        if INDEX_LINK:
            indexurl = f"{INDEX_LINK}/{file_upload}"
            tam_link = requests.utils.requote_uri(indexurl)
            print(tam_link)
        ...
        await asyncio.sleep(EDIT_SLEEP_TIME_OUT)
        await messa_ge.reply_text(f"File : {file_upload}\n\nHas been upload successfully to Google Drive <a href='tg://user?id={g_id}'>👌</a>\n\n💾 Size: {gjay}", reply_markup=button_markup)
